...
# subtype 1
classtype=ifelse(clinical$Subtype==1,1,2)
survobj=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype)
model=survfit(survobj~1,data = clinical)

# subtype 2
classtype2=ifelse(clinical$Subtype==2,1,2)
survobj2=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype2)
model2=survfit(survobj2~1,data = clinical)

# subtype 3
classtype3=ifelse(clinical$Subtype==3,1,2)
survobj3=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype3)
model3=survfit(survobj3~1,data = clinical)

# subtype 4
classtype4=ifelse(clinical$Subtype==4,1,2)
survobj4=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype4)
model4=survfit(survobj4~1,data = clinical)
