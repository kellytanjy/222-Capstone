...
# subtype 1
subtype1 = clinical[clinical$Subtype==1,]
survobj=Surv(as.numeric(subtype1$`Survival (days)`)/30, subtype1$VITALSTATUS==1)
model=survfit(survobj~1,data = subtype1)

# subtype 2
subtype2 = clinical[clinical$Subtype==2,]
survobj2=Surv(as.numeric(subtype2$`Survival (days)`)/30, subtype2$VITALSTATUS==1)
model2=survfit(survobj2~1,data = subtype2)

# subtype 3
subtype3 = clinical[clinical$Subtype==3,]
survobj3=Surv(as.numeric(subtype3$`Survival (days)`)/30, subtype3$VITALSTATUS==1)
model3=survfit(survobj3~1,data = subtype3)

# subtype 4
subtype4 = clinical[clinical$Subtype==4,]
survobj4=Surv(as.numeric(subtype4$`Survival (days)`)/30, subtype4$VITALSTATUS==1)
model4=survfit(survobj4~1,data = subtype4)

plot(model,conf.int=F,xlab = "Month", ylab = "Survival", main = "Survival Plot of 4 Subtypes", col = "black")
lines(model2,col="red",conf.int = F)
lines(model3,col="blue",conf.int = F)
lines(model4,col="green",conf.int = F)
