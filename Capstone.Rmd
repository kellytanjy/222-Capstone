...
=======
>>>>>>> 3737a4e5ae522560254448d8b48d2f31ea2e5cb2
```
...
library(samr)
##for class1 analysis
clusterfor1=clust_labels
clusterfor1=ifelse(clusterfor1$class==1,1,2)
samfit=SAM(x=dat,y=clusterfor1,resp.type = 'Two class unpaired')
plot(samfit)
```
...
#Classical
classtype=ifelse(clinical$Subtype=='Classical',1,2)
survobj=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype)
model=survfit(survobj~1,data = clinical)
plot(model)
```

```{r}
#Neural
classtype=ifelse(clinical$Subtype=='Neural',1,2)
survobj=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype)
model2=survfit(survobj~1,data = clinical)
plot(model2)
```

```{r}
#Proneural
classtype=ifelse(clinical$Subtype=='Proneural',1,2)
survobj=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype)
model3=survfit(survobj~1,data = clinical)
plot(model3)
```
...
```{r}
#Mesenchymal
classtype=ifelse(clinical$Subtype=='Mesenchymal',1,2)
survobj=Surv(as.numeric(clinical$`Survival (days)`)/30, classtype)
model4=survfit(survobj~1,data = clinical)
plot(model4,conf.int=FALSE)
lines(model3,col="red",conf.int = FALSE)
lines(model2,col="blue",conf.int = F)
lines(model,col="green",conf.int = F)
```
